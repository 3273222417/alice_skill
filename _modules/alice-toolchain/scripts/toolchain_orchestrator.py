#!/usr/bin/env python3
"""Validate and route the Alice工具链 independent toolchain contract."""
...
def validate(manifest: dict[str, Any]) -> dict[str, int | str]:
    need(manifest.get("schema") == 1 and manifest.get("version") == "3.1.0", "schema/version mismatch")
    ...
    return {"schema": 1, "version": "3.1.0", "domains": 17, "nodes": 181,
...
def dispatch(prompt: str, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    activation_terms = [manifest["activation"]["exact"], *manifest["activation"].get("aliases", [])]
    if text in {normalize(value) for value in activation_terms}:
        return {"matched": True, "activation": "taskroute", "full_chain": True, "coverage": 181,
...
def self_test(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    cold = dispatch("任务路由", manifest)
    need(cold.get("full_chain") is True and cold.get("coverage") == 181 and len(cold["chain"]) == 17, "activation failed")
    english = dispatch("task route", manifest)
    need(english.get("full_chain") is True and english.get("coverage") == 181, "English activation failed")
    need(dispatch("任务路由！", manifest).get("full_chain") is False, "activation must remain exact")
    cases = {
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate and route the Alice工具链 toolchain contract")
