...
| # | 原问题 | 现状 |
|---|---|---|
| 1 | `mock8880.py` 写死 `C:\wz_evoc\logs\`，目录不存在直接崩 | 多候选探测日志路径 + 自动建目录；新增 `--port/--mode/--log` |
| 2 | 11 个文件硬编码 `C:\Users\alicewe\...`、`C:\wz_evoc`、`F:\alice破甲\...` | 全部改为**相对脚本位置推导** + 命令行覆盖；可整包搬到任意目录/中文路径 |
